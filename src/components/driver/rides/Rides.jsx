...
function Rides() {
  ...
  const token = DriverDetails?.token;
    ...
    (async function  () {
      setLoader(true)
      const response = await availableRides(driverid, token)
      console.log((response?.data,'iddddddddd'))
      setLoader(false) 
        setid(response?.data?._id)
      const res = await tripFind(id) 
      setDatas(res?.data)
 ...
 function startdrive() {
  ...
  return (
    <>
      ...
      {/*  */}
      { modal  ? <ConfirmModal know={know} setmodal={setmodal} setstatus={setstatus}/> :""}
              ...
              <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full rounded-[50px] text-center shadow-[1px_1px_2px_2px_grey]">
                  ...
                  <>
                   ...
                   {(rideDatas?.bookingStatus === 'confirmed') && (rideDatas?.Arraivalstatus !== 'way' )? <Ride setStartmodal={setStartmodal} />:(rideDatas?.bookingStatus === 'Pending')?<div className="flex flex-wrap justify-between pl-20">
                    <Button setmodal={setmodal}  setknow={setknow}/>
                    <Reject setmodal={setmodal}  setknow={setknow}/>
                   </div>:(rideDatas?.Arraivalstatus === 'way' ) && (rideDatas?.bookingStatus === 'confirmed') ? <button className="bg-green-600 rounded-lg text-white px-2 py-2" onClick={startdrive}>start drive</button> :<div><button className="bg-red-600 rounded-lg text-white px-2 py-2 cursor-not-allowed">Rejected</button></div>}
                  </>
                </div>
              </div>
